...
describe("AudioInputNode", () => {
  const nodeId = "audio-input";
  const node = new AudioInputNode({ nodeId } as unknown as Node);
  ...
  const blob = { data: new Uint8Array([1, 2, 3]), mimeType: "audio/mpeg" };
  ...
  it("errors when no value is provided", async () => {
    ...
    expect(result.error).toBe("No audio provided");
  });
