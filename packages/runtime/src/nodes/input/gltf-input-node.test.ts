...
import { GltfInputNode } from "./gltf-input-node";

describe("GltfInputNode", () => {
  const nodeId = "gltf-input";
  const node = new GltfInputNode({ nodeId } as unknown as Node);
  ...
  it("errors when no value is provided", async () => {
    ...
    expect(result.error).toBe("No GLTF model provided");
  });
