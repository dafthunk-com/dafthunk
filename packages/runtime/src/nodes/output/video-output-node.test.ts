...
import { VideoOutputNode } from "./video-output-node";

describe("VideoOutputNode", () => {
  const nodeId = "output-video";
  const node = new VideoOutputNode({ nodeId } as unknown as Node);
  ...
  it("errors when data is not a Uint8Array", async () => {
    const result = await node.execute(
      createContext({ value: { data: "not-bytes", mimeType: "video/mp4" } })
    );
