...
export class GeoTiffQueryNode extends ExecutableNode {
  public static readonly nodeType: NodeType = {
    ...
    type: "geotiff-query",
    description:
      "Query a Cloud Optimized GeoTIFF with a bounding box and return an image.",
    tags: ["3D", "Geo"],
    icon: "search",
    inputs: [
      {
      ...
      {
        name: "bbox",
        type: "json", // [minX, minY, maxX, maxY] in WGS84 - pre-validated
        description: "Pre-validated bounding box coordinates",
        required: true,
      },
      {
        name: "width",
  ...
  public async execute(context: NodeContext): Promise<NodeExecution> {
    ...
    try {
      const imageData = await this.queryGeoTiff(
        url,
