...
import { createDatabase, getEmailByHandle } from "../db";
...
export interface EmailAgentRunRequest {
  ...
  /** Org email handle (local part) to send from; resolved to an email id. */
  fromHandle: string;
...
export class EmailAgentRunner extends DurableObject<Bindings> {
  ...
  async start(request: EmailAgentRunRequest): Promise<{ status: string }> {
    ...
    const email = await getEmailByHandle(db, request.fromHandle.toLowerCase());
    if (!email || email.organizationId !== request.organizationId) {
      await this.fail(
        request,
        `Sending address '${request.fromHandle}' not found for this organization`
      );
      return { status: "error" };
    }
