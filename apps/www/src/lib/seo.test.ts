...
function inRange(s: string): boolean {
  return (
    s.length >= META_DESCRIPTION_MIN && s.length <= META_DESCRIPTION_MAX
  );
}

describe("metaDescription bounds on authored entities", () => {
  it.each(categories.map((c) => [c.id, c] as const))(
    "categories/%s has a metaDescription in [110, 160]",
    (_id, category) => {
  ...
  it.each(workflows.map((w) => [w.id, w] as const))(
    "workflows/%s has a metaDescription in [110, 160]",
    (_id, workflow) => {
  ...
  it.each(alternatives.map((a) => [a.id, a] as const))(
    "alternatives/%s has a metaDescription in [110, 160]",
    (_id, alt) => {
